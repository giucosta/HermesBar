...
namespace BLL.User
{
    public class EmailBLL : IDisposable
    {
		...
		public EmailBLL(string host, int port, string email,string password): this(host, port, email, password, false){}
		public EmailBLL(string host, int port, string email,string password, bool secure)
		{
			Host = host;
			Port = port;
			Email = email;
